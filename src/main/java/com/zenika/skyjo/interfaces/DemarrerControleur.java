...
public class DemarrerControleur {
    ...
    @PostMapping
    public ResponseEntity<Manche> demarrerUneNouvellePartie(@Valid @RequestBody ListeDesJoueursDto listeDesJoueursDto) {
        Manche manche = skyjoLogique.preparerUneManche(listeDesJoueursDto.joueurs());
        return ResponseEntity.created(URI.create("/manches/" + manche.getId())).body(manche);
    }

    @PutMapping("/{mancheId}/engager")
    public ResponseEntity<Manche> retournerDeuxCartes(@PathVariable String mancheId,
        ...
        return ResponseEntity.ok().body(manche);
    }
}
